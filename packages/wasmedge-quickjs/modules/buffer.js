...
function dew() {
  ...
  function byteLength(string, encoding) {
    ...
    if (!mustMatch && len === 0)
      return 0 // Use a for loop to avoid recursion
  ...
  Buffer.prototype.copy = function copy(target, targetStart, start, end) {
    ...
    if (!targetStart)
      targetStart = 0
    if (end > 0 && end < start)
      end = start // Copy 0 bytes; we're done

    if (end === start)
      return 0
    if (target.length === 0 || this.length === 0)
      return 0 // Fatal error conditions
    ...
    if (start < 0 || start >= this.length)
      throw new RangeError('Index out of range')
    if (end < 0)
      throw new RangeError('sourceEnd out of bounds') // Are we oob?
  ...
  E('ERR_INVALID_ARG_TYPE', (name, actual) => {
