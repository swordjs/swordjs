...
export class Response {
  ...
  async #readChunk() {
    while (true) {
      const chunk = this.#chunkBuff.parseChunk()
...
export async function fetch(input, init = {}) {
  ...
  const addr = net.nsloopup(url.host, `${url.port}`)[0]

  const s = await net.WasiTcpConn.connect(addr)
  const req = new httpx.WasiRequest()
...
class HttpConn {
  ...
  respondWith(body, resp_header) {
    ...
    const resp = new httpx.WasiResponse()
  ...
  write(chunk) {
    if (chunk) {
      const conn = this.#chunkBuffer ?? this.#chunk ?? this.socket
...
class ServerImpl extends EventEmitter {
  ...
  listen(...args) {
    ...
    if (cb != null) {
      // @ts-expect-error change EventEmitter's sig to use CallableFunction
    ...
    if (listener !== undefined) {
      ...
      try {
        while (this.#listening) {
          const tcp_conn = await listener.accept()
          try {
            const httpConn = new HttpConn(tcp_conn)
