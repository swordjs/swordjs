...
import { validateBoolean, validateFunction, validateInteger } from '../internal/validators'
import { Stats, getValidMode, getValidatedPath, stringToFlags, validateEncoding } from '../internal/fs/utils'
import * as errors from '../internal/errors'
import { hideStackFrames } from '../internal/errors'
import { fs as constants } from '../internal_binding/constants'
import { customPromisifyArgs, kCustomPromisifiedSymbol, promisify } from '../internal/util'
import { cpFn } from '../internal/fs/cp/cp'
import cpSyncFn from '../internal/fs/cp/cp-sync'
import { ReadStream, WriteStream, createReadStream, createWriteStream } from '../internal/fs/stream'
...
function wasiFsSyscallErrorMap(err, syscall, path, dest) {
  ...
  const code = (codeToErrorMsg[err.code] === undefined) ? (`E${err.code}`) : err.code
...
function statSync(path, options = { bigint: false, throwIfNoEntry: true }) {
  ...
  try {
    const stat = binding.statSync(path)
...
function lstatSync(path, options = { bigint: false, throwIfNoEntry: true }) {
  ...
  try {
    const stat = binding.lstatSync(path)
    if (options.bigint === true)
      return new BigIntStats(stat)
    else
      return new Stats(stat)
...
function fstatSync(fd, options = { bigint: false, throwIfNoEntry: true }) {
  ...
  try {
    const stat = binding.fstatSync(fd)
    if (options.bigint === true)
      return new BigIntStats(stat)
    else
      return new Stats(stat)
...
function mkdir(path, options, callback) {
  ...
  setTimeout(() => {
    try {
      const newPath = mkdirSync(path, options)
...
function realpathSync(path, options = { encoding: 'utf8' }) {
  ...
  validateEncoding(options.encoding, 'encoding')
  const useBuffer = options.encoding === 'buffer' || options.encoding === 'Buffer'
  const stat = lstatSync(path)
  if (stat != null) {
    if (!stat.isSymbolicLink()) {
      const res = normalize(path)
      if (!useBuffer)
        return res
      else
...
function copyFileSync(src, dest, mode = 0) {
  ...
  if (mode & constants.COPYFILE_EXCL === constants.COPYFILE_EXCL) {
    if (existsSync(dest)) {
      const e = new Error(`EEXIST: file already exists, copyfile '${src}' -> '${dest}'`)
      e.code = 'EEXIST'
      e.syscall = 'copyfile'
...
function readSync(fd, buffer, offset, length, position) {
  ...
  position = Number(position)
  try {
    const data = binding.freadSync(fd, position, length)
    if (buffer instanceof Buffer)
...
function openSync(path, flag = 'r', mode = 0o666) {
  ...
  try {
    const fd = binding.openSync(path, flag, mode)
    return fd
  }
  catch (err) {
...
function readFile(path, option, callback) {
  ...
  const buf = Buffer.alloc(Math.max(len, 1))
...
function readlinkSync(path, option) {
  ...
  validateEncoding(option.encoding, 'encoding')
  try {
    const res = binding.readlinkSync(path)
...
function readlink(path, option, callback) {
  ...
  setTimeout(() => {
    try {
      const res = readlinkSync(path, option)
      callback(null, res)
    }
    catch (err) {
      callback(err)
...
function writeSync(fd, buffer, offset, length, position) {
  ...
  try {
    const len = binding.fwriteSync(fd, position, buffer.buffer.slice(offset, offset + length))
    return len
  }
  catch (e) {
...
function writeFile(file, data, options, callback) {
  ...
  if (!(buffer instanceof Buffer)
    ...
    const enc = new TextDecoder(options.encoding)
...
function writeDataCheck(data) {
  const ok = data && (typeof (data) === 'string' || isArrayBufferView(data))
  if (!ok)
...
function writevSync(fd, buffer, position = -1) {
  ...
  try {
    const len = binding.fwriteSync(fd, position, Buffer.concat(buffer).buffer)
    return len
  }
  catch (err) {
...
class Dir {
  ...
  #fetch() {
    ...
    if (this.#idx === this.#dataBuf.length && !this.#fin) {
      try {
        const data = binding.freaddirSync(this.#fd, this.#cookie)
...
function readdir(path, options, callback) {
  ...
  setTimeout(async () => {
    try {
      const data = []
      const dir = opendirSync(path, { encoding: options.encoding })
